...
Em 2026-05-09, foi iniciada a fase de reconcilia��o do hist�rico de migrations Supabase para o projeto `lasocsneburvtxqgqhie`.
...
- vers�o do Supabase CLI
- migration list antes da reconcilia��o
- dry-run antes da reconcilia��o
- dump remoto do schema
- tentativa de db diff
- auditoria de tabelas remotas versus migrations locais

O `db diff --linked --schema public` falhou ao aplicar `002_metadata_and_constraints.sql` em uma shadow database, porque `campaign_summary` n�o � criada pelas migrations locais.

A auditoria identificou tabelas existentes no remoto, mas n�o criadas localmente:
...
N�o executar `supabase migration repair` nesta fase, pois as migrations locais `001` a `007` n�o representam um baseline completo do schema remoto.

Pr�xima etapa recomendada:

`v1.3.2 Supabase Baseline Recovery`, com objetivo de recuperar ou reconstruir uma migration baseline m�nima antes de retomar a reconcilia��o do hist�rico.

## Supabase migration history reconciliation conclu�da

Em 2026-05-09, a reconcilia��o do hist�rico de migrations Supabase foi conclu�da para o projeto `lasocsneburvtxqgqhie`.

Contexto:

A fase anterior identificou que o remoto possu�a timestamps antigos em `supabase_migrations.schema_migrations`, enquanto o reposit�rio local possu�a migrations numeradas.

Ap�s auditoria, foi constatado que as migrations locais n�o reconstru�am o schema p�blico completo, pois dependiam de um baseline pr� existente.

Corre��es locais realizadas:
...
N�o foi executado `db push` real.

A reconcilia��o alterou apenas o hist�rico remoto de migrations via `supabase migration repair`, ap�s o schema p�blico reconstru�do localmente ter sido validado contra o remoto.
